...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      //老师的写法(老师的写法没有传参，而是通过event获取value) 
      /* changeskuNum(e){
        // console.log(isNaN(e.target.value)) //其实不用*1也能直接判断是不是NaN
        // 用户输入进来的文本*1
      ...
      async addShopCart(){
        try{
          ...
          sessionStorage.setItem('SKUINFO',JSON.stringify(this.skuInfo)) //这个会话存储是为那个步骤哪个页面存储的
        }catch(error){
          alert(error.message)
        }
      }
