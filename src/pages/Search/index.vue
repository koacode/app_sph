            ...
            <div class="navbar-inner filter">
              ...
              <ul class="sui-nav">
                <li :class="{active:isOne}" @click="changeOrder('1')">
                  <a>综合<span v-show="isOne" class="iconfont" :class="{'icon-upload':isUp,'icon-i-down':isDown}"></span></a>
                </li>
                <li :class="{active:isTwo}" @click="changeOrder('2')">
                  <a>价格<span v-show="isTwo" class="iconfont" :class="{'icon-upload':isUp,'icon-i-down':isDown}"></span></a>
                </li>
              </ul>
            </div>
          </div>
  ...
  export default {
    ...
    methods:{
      ...
      attrInfo(attr,attrValue){
        ...
        // 数组去重，判断数组中是否含有该元素，不能重复添加
        // if语句花括号中的内容如果只有一行代码，可以省略花括号
        if(this.searchParams.props.indexOf(props)==-1) this.searchParams.props.push(props)
        this.getData()
      },
      // 删除商品售卖的属性，通过传入点击的数组索引值删除数组中的元素
      removeAttr(index){
      ...
      /* changeOrder(flag){
        if(this.searchParams.order.split(':')[0] == flag){
          ...
          // 这里其实不需要两个判断，因为我已经点了不是本来高亮的那个元素，要把它变为高亮
          // 最初变为高亮时，系统默认它要降序，因此直接变为降序就好，不需要判断了
          // 这里判断的话会导致会按之前的状态进行显示，而不是默认的降序，其实这样判断是正常的，但是系统默认是降序
          // if(this.isUp){
          //   this.searchParams.order = `${flag}:desc`
          // }else{
          //   this.searchParams.order = `${flag}:asc`
          // }
          // 这样就能保证每次点那个不高亮的元素变为高亮之后都是降序的
          this.searchParams.order = `${flag}:desc`
          // 修改完参数之后要记得发请求，不然白改了
          this.getData()
        }
